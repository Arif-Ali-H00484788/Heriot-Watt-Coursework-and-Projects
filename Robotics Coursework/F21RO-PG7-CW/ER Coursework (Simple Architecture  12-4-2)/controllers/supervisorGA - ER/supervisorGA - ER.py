...
class SupervisorGA:
    def __init__(self):
        ...
        #Initialize Light source node
        self.light_source = self.supervisor.getFromDef("LightCue")
        if self.light_source is None:
            sys.stderr.write("No DEF LIGHT_SOURCE node found in the current world file\n")
            sys.exit(1)
    ...
    def evaluate_genotype(self,genotype,generation):
        ...
        while currentInteraction < numberofInteractionLoops:
            ...
            #Set light Cues
            self.light_source.getField("on").setSFBool(True)
            light_value_right = self.light_source.getField("on").getSFBool()
            ...
            Trial_Type_A_Reward_Zone_Position_Right = [  0.360178 , -0.0000844974 , -0.157513 ]
            Trial_Type_A_Reward_Zone_Position_Left  = [ -0.360178 , -0.0000844974 , -0.157513 ]
            Trial_Type_A_Robot_Position = self.trans_field.getSFVec3f()
            Trial_Type_A_Reward_Threshold = 0.1 
                      
            # Calculate the Euclidean distance to the reward zone
            Trial_Type_A_Distance_To_Reward_Right = np.linalg.norm(np.array(Trial_Type_A_Robot_Position) - np.array(Trial_Type_A_Reward_Zone_Position_Right))
            Trial_Type_A_Distance_To_Reward_Left = np.linalg.norm(np.array(Trial_Type_A_Robot_Position) - np.array(Trial_Type_A_Reward_Zone_Position_Left))
            # If the robot is at the end of the maze
            
            if (light_value_right == True):
                # Check if the robot is at the end of the maze 
                if Trial_Type_A_Distance_To_Reward_Right <= Trial_Type_A_Reward_Threshold:
                    fitness = fitness + 10
                    print("Trial Type-A (Right) :- Reward zone reached at right end")
                # Checks if robot is towards the end of the maze on the right
                elif Trial_Type_A_Distance_To_Reward_Left <= Trial_Type_A_Reward_Threshold:
                    fitness = fitness - 5
                    print("Trial Type-A (Right) :- Penalty zone reached at left end")
                else:
                    print("Trial Type-A (Right) :- Robot has failed to reach any reward zone")
                    fitness = fitness - 5
            ...
            #Set light Cues
            self.light_source.getField("on").setSFBool(False)
            light_value_left = self.light_source.getField("on").getSFBool()
            # Evaluation genotype 
            self.run_seconds(self.time_experiment)
            ...
            Trial_Type_B_Reward_Zone_Position_Right = [  0.360178 , -0.0000844974 , -0.157513 ]
            Trial_Type_B_Reward_Zone_Position_Left  = [ -0.360178 , -0.0000844974 , -0.157513 ]
            Trial_Type_B_Robot_Position = self.trans_field.getSFVec3f()
            Trial_Type_B_Reward_Threshold = 0.1 
                      
            # Calculate the Euclidean distance to the reward zone
            Trial_Type_B_Distance_To_Reward_Right = np.linalg.norm(np.array(Trial_Type_B_Robot_Position) - np.array(Trial_Type_B_Reward_Zone_Position_Right))
            Trial_Type_B_Distance_To_Reward_Left = np.linalg.norm(np.array(Trial_Type_B_Robot_Position) - np.array(Trial_Type_B_Reward_Zone_Position_Left))
            # If the robot is at the end of the maze
            
            if (light_value_left == False):
                # Check if the robot is at the end of the maze 
                if Trial_Type_B_Distance_To_Reward_Left <= Trial_Type_B_Reward_Threshold:
                    fitness = fitness + 5
                    print("Trial Type-B (Left) :- Reward zone reached at left end")
                # Checks if robot is towards the end of the maze on the right
                elif Trial_Type_B_Distance_To_Reward_Right <= Trial_Type_B_Reward_Threshold:
                    fitness = fitness - 3
                    print("Trial Type-B (Left) :- Penalty zone reached at right end")
                else:
                    print("Trial Type-B (Left) :- Robot has failed to reach any reward zone")
                    fitness = fitness - 5
